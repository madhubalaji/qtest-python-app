...
def _get_priority_filter_options():
    """Get priority filter options in Japanese."""
    return ["すべて", "低", "中", "高"]


def _map_japanese_priority_to_english(japanese_priority):
    """Map Japanese priority to English."""
    priority_map = {"すべて": "All", "低": "Low", "中": "Medium", "高": "High"}
    return priority_map.get(japanese_priority, "All")


def _map_english_priority_to_japanese(english_priority):
    """Map English priority to Japanese."""
    priority_map = {"low": "低", "medium": "中", "high": "高"}
    return priority_map.get(english_priority.lower(), english_priority)


def _filter_tasks(tasks, show_completed, filter_priority):
    ...
    english_priority = _map_japanese_priority_to_english(filter_priority)
...
def _display_task_row(task, task_service):
    """Display a single task row with all controls."""
    ...
    with col2:
        ...
        priority_japanese = _map_english_priority_to_japanese(task.priority)
...
def _handle_delete_confirmation(task_service):
...
def display_tasks_page(task_service):
    ...
    with col2:
        filter_priority = st.selectbox(
            "優先度でフィルター",
            _get_priority_filter_options()
        )

    # Get and filter tasks
    tasks = task_service.get_all_tasks(show_completed=True)
    tasks = _filter_tasks(tasks, show_completed, filter_priority)
    ...
    for task in tasks:
        with st.container():
            _display_task_row(task, task_service)
            st.divider()

    # Handle delete confirmation
    _handle_delete_confirmation(task_service)
...
def _display_search_result(task):
...
def _display_task_detail(task_service):
    """Display detailed view of a selected task."""
...
def _handle_delete_from_detail(task_service):
...
def search_tasks_page(task_service):
    ...
    if keyword:
        ...
        if not results:
            ...
            for task in results:
                with st.container():
                    _display_search_result(task)
                    st.divider()

    # Display task details if selected
    _display_task_detail(task_service)

    # Handle delete confirmation from task detail view
    _handle_delete_from_detail(task_service)
