            ...
            with col4:
                if st.button("🗑️", key=f"delete_{task.id}"):
                    ...
                    if st.session_state.delete_confirmation == task.id:
                        ...
                        st.warning(f"Are you sure you want to delete '{task.title}'? Click the delete button again to confirm.")
        ...
        try:
            ...
            with col2:
                if st.button("Delete Task"):
                    if 'delete_confirmation_view' not in st.session_state:
                        st.session_state.delete_confirmation_view = False
                    
                    if st.session_state.delete_confirmation_view:
                        # User has confirmed deletion
                        task_service.delete_task(task.id)
                        ...
                        # Ask for confirmation
                        st.session_state.delete_confirmation_view = True
                        st.warning(f"Are you sure you want to delete '{task.title}'? Click the delete button again to confirm.")
