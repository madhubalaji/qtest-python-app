...
def display_tasks_page(task_service):
    ...
    for task in tasks:
        with st.container():
            ...
            with col4:
                # MBSOURCE: Delete button with confirmation using session state
                if st.button("🗑️", key=f"delete_{task.id}", help="Delete task"):
                    st.session_state[f"confirm_delete_{task.id}"] = True
                    st.experimental_rerun()
                
                # MBSOURCE: Show confirmation dialog if delete was clicked
                if st.session_state.get(f"confirm_delete_{task.id}", False):
                    ...
                    with col_yes:
                        if st.button("Yes", key=f"confirm_yes_{task.id}"):
                            try:
                                ...
                                # MBSOURCE: Clean up session state after successful deletion
                    ...
                    with col_no:
                        if st.button("No", key=f"confirm_no_{task.id}"):
                            # MBSOURCE: Cancel deletion and clean up session state
                            if f"confirm_delete_{task.id}" in st.session_state:
                                del st.session_state[f"confirm_delete_{task.id}"]
                            st.experimental_rerun()
...
def search_tasks_page(task_service):
    ...
    if hasattr(st.session_state, 'task_to_view'):
        try:
            ...
            with col2:
                # MBSOURCE: Delete button in task details view
            ...
            # MBSOURCE: Confirmation dialog for delete in task details
            if st.session_state.get(f"confirm_delete_detail_{task.id}", False):
                st.warning(f"Are you sure you want to delete '{task.title}'?")
                col_yes, col_no = st.columns(2)
                
                with col_yes:
                    if st.button("Yes, Delete", key=f"confirm_detail_yes_{task.id}"):
                        try:
                            deleted_task = task_service.delete_task(task.id)
                            st.success(f"Task '{deleted_task.title}' deleted successfully!")
                            # MBSOURCE: Clean up session state after deletion
